...
| `value`, `passed` | nullable **парой**: обе null = метрика не посчитана, вердикт `unknown`. Это не провал и не проход — это список того, чего не хватает для решения |
| `source` | `qlab`/`imported` — вердикты, вынесенные нами, и импортированные исторические |
| `data_range_start`, `data_range_end` | **на каких данных вынесен.** Обязательны при `source='qlab'`, nullable только для `imported`: в старых досье диапазон указан не всегда, а выдумывать даты нельзя — на них стоит логика воскрешения |
| `decided_at`, `note` | |

**Три законных вида строки в `verdict`** — различать обязательно, иначе в реестр попадают
выдуманные пороги:

| вид | `value` | `comparator`/`threshold` | `passed` | когда |
|---|---|---|---|---|
| **решение** | есть | есть | есть | правило применено к посчитанной метрике |
| **неизвестно** | null | есть | null | правило есть, метрика не посчитана |
| **измерение** | есть | **null** | null | число измерено, но формального порога не было — только `imported` |

Третий вид — про историю. В старых досье полно измеренных чисел («Шарп 0.77»), под которыми
не стояло формализованного критерия: решение принимал человек. Записать такую строку как
решение можно только придумав порог — а придуманный порог потом всплывёт в запросе
«почти прошёл» как настоящий промах и повлияет на распределение денег. Поэтому измерение
хранится без порога, `rule_id = "unidentified"`, и в отбор не участвует.

**Версия правил у исторических вердиктов — сентинел `frab-legacy`**: критерии
дофициального периода формализованы не полностью, файла с такой версией нет и не будет.
Сопоставление с текущими правилами идёт по `rule_id` через секцию `retired`.
Вердикт без диапазона данных не участвует в автоматическом воскрешении: не зная даты
отказа, нельзя определить, какие данные новые.
...
Движок правил — **чистая функция** `evaluate(metrics, ruleset) -> EvaluationResult`.
Никакой агент вердикт не выносит.

Агрегат **fail-closed**: `overall_passed=True` только если ни одно правило не провалено
**и** все метрики посчитаны (`decisive=True`). Кандидат, про которого ничего не считали,
отбор не проходит — иначе пустые метрики означали бы допуск.
